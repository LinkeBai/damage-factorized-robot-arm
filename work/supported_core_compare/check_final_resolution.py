"""Same stiff-contact development replay at 0.25 ms for final resolution comparison."""
...
def main():
    folder=OUT/'solver-development'; target=folder/'stiff_contact_limit_025ms.json'
    if target.exists(): raise FileExistsError(target)
    write(folder/'final-resolution-protocol.json', {'scope':'Same contact/limit parameters and all 320 development inputs; resolution check only',
        'dt_s':.00025,'duration_s':.25,'script_sha256':sha(__file__),
        'coarse_report_sha256':sha(folder/'stiff_contact_limit_05ms.json')})
    ...
    with np.load(folder/'stiff_contact_limit_05ms.npz') as a: coarse=a['traces']
    rows=[]; traces=[]
    for i,record in enumerate(records):
        lock,profile=record['lock'],record['profile']
        m=supported.make_model(lock,profile);base.configure(m,'stiff_contact_limit_1ms');m.opt.timestep=.00025
        ...
        for step in range(1001):
            if step:
                d.ctrl[:]=actions[i,(step-1)//200];mujoco.mj_step(m,d)
    ...
    np.savez_compressed(folder/'stiff_contact_limit_025ms.npz',traces=np.asarray(traces))
    report={'rows':rows,'trace_sha256':sha(folder/'stiff_contact_limit_025ms.npz'),
