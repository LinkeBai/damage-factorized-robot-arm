...
Level-A现场后处理已合并为一条 fail-fast 命令，依次执行 preflight、冻结/完成日志
...
整个长期目标新增统一 fail-closed 完成审计：仿真 9/9、模型来源/大指标归因、PDF
匿名与页数、真机 preflight、冻结/完成日志一致性、intact/D2/D3 各至少 10 条且
原始文件通过、以及独立复评真实核验后达到 4.0/5，全部为 required。固定预抓取
夹取为 optional，不能补偿 Push 或评分缺口。当前审计应输出 INCOMPLETE，直到
真实文件与最终复评 JSON 到位；不得因为测试全绿就误标整个 goal 完成。
