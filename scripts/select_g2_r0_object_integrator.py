"""Select object position/velocity consistency using validation domains only."""
...
from scripts.run_bt_dpwm_gate_y0 import (
    aggregate_topology_losses, cached_collect, object_losses_per_trajectory,
    topology_group_indices,
)
...
def main():
    ...
    parser.add_argument("--blends", nargs="+", type=float, default=[0, .25, .5, .75, 1])
    ...
    batch = _batch(trajectories, device)
    groups = topology_group_indices(trajectories, device)
    candidates = []
    with torch.no_grad():
        for blend in args.blends:
            ...
            for horizon in args.horizons:
                per = object_losses_per_trajectory(
                    model, batch, horizon, use_topology=True)
                ...
                losses.append(value)
            candidates.append({"blend": blend, "mean_validation_loss": float(
                torch.stack(losses).mean()), "per_horizon": per_horizon})
    selected = min(candidates, key=lambda item: item["mean_validation_loss"])
    result = {"version": "g2_r0_object_integrator_selection_v1",
