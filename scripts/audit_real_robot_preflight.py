...
def audit(manifest_path: Path, schedule_path: Path, require_paths: bool = True) -> dict:
    errors: list[str] = []
    warnings: list[str] = []
    manifest = yaml.safe_load(manifest_path.read_text(encoding="utf-8"))
    schedule_hash = sha256(schedule_path)
    if schedule_hash != EXPECTED_SHA256:
    ...
    return {
        ...
        "warnings": warnings,
        "authorization": (
            "FORMAL_TRIALS_MAY_START" if not errors
...
def main() -> None:
    ...
    payload = audit(args.manifest, args.schedule, not args.skip_path_existence)
